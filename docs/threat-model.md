...
| **Recovery** | There is no escrow or recovery path for a lost PSK. `keygen` on the sink plus redistribution is the recovery procedure. Unlike `rotate-key`, the sink accepts only the new key from that moment, so sources still holding the old key stop connecting. (`cmd/agentpantry`, README "Rotating the pre-shared key") | On suspected exposure, prefer `rotate-key` for zero-downtime rotation, finish it promptly, and delete any `psk.key.bak.*` files: they are live retired key material. |
