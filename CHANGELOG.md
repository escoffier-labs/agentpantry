...
- `agentpantry pair` bootstraps the long-lived PSK with a one-time SPAKE2 short code, as an alternative to copying `psk.key`. The session-salt handshake and HKDF session keys are unchanged.
=======
...
>>>>>>> 6676962 (fix: harden pairing bind, attempt budget, and RFC 9382 order)
